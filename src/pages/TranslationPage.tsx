...
export const TranslationPage: React.FC = () => {
  ...
  const [result, setResult] = useState<{ translatedText: string; confidence: number; isExact: boolean }>({
  ...
  const handleSwap = () => {
    ...
    if (result.translatedText) {
      ...
      const res = translationService.translateText(result.translatedText, targetLang, temp);
      setResult(res);
    }
  };
  ...
  const handleSpeak = (text: string, langCode: string) => {
    ...
    speechSynthesisService.speak(text, { lang: langObj?.speechCode });
    showToast(`Speaking in ${langObj?.name || 'selected voice'}`, 'info');
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8" id="translation-page-root">
      {/* Header */}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-[200px]">
          ...
          <select
            value={sourceLang}
            onChange={e => setSourceLang(e.target.value as SpokenLanguageCode)}
        ...
        <button
          onClick={handleSwap}
          className="p-2.5 bg-slate-100 hover:bg-teal-50 text-slate-700 hover:text-teal-700 rounded-xl border border-slate-200 transition-colors shadow-2xs"
        ...
        <div className="flex items-center gap-2 flex-1 min-w-[200px]">
          ...
          <select
            value={targetLang}
            onChange={e => setTargetLang(e.target.value as SpokenLanguageCode)}
            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-semibold text-slate-900 focus:ring-2 focus:ring-teal-500 cursor-pointer"
        ...
        <div className="bg-teal-950 text-white rounded-3xl border border-teal-800 shadow-xl p-6 flex flex-col justify-between space-y-4">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-teal-900/80">
            ...
            <span className="text-xs text-slate-400 font-medium">
              Target: {SPOKEN_LANGUAGES.find(l => l.code === targetLang)?.name}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="pt-4 space-y-3">
        ...
        <SignAvatarViewer text={inputText} signLanguage={signLang} autoPlay={false} />
      </div>
    </div>
  );
};
